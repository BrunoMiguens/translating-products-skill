...
Consume the core draft, source context, surface, glossary, protected terms, placeholder types/examples, and the project's gender-inclusive language convention. A goal such as “address readers inclusively” is not yet a convention: the context must name the accepted grammatical or orthographic strategy, such as neutral restructuring, doubled forms, separators, or a declared combination. If that strategy is absent, return one focused question through the orchestrator and withhold all target copy instead of silently choosing a house style.
...
5. Apply the configured inclusive-address convention consistently without making compact UI unreadable or long-form prose repetitive.
